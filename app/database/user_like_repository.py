...
class UserLikeRepository:
    async def _check_correct_db_request(self, user_id: int, other_user_id: int, session):
    ...
    async def add_like(self, user_id: int, other_user_id: int, session):
        await self._check_correct_db_request(user_id, other_user_id, session)
        new_like = UserLikes(user_id=user_id, 
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail="Не удалось добавить лайк")

        mutual_like_exists = await self._check_mutual_like(user_id, other_user_id, session)
    ...
    async def _check_mutual_like(self, user_id: int, other_user_id: int, session) -> bool:
    ...
    async def _remove_mutual_likes(self, user_id: int, other_user_id: int, session):
