...
class UserInfoRepository:
    async def get_user_data_by_email(
        ...
        results = await session.execute(
                select(
                    ...
                    UserAccounts.hashed_password,
                    UserMails.email
                ).join(
                    UserMails,
                    UserAccounts.id == UserMails.user_id
    ...
    async def get_user_names_by_id(
        ...
        results = await session.execute(
                select(
                    ...
                    UserMails,
                    UserNames.user_id == UserMails.user_id
                ).filter(
                    UserMails.user_id == id
                )
            )
        user_data = results.first()
        if not user_data:
    ...
    async def get_user_list(
        ...
        query = select(
                    ...
                    UserNames.first_name,
                    UserNames.last_name,
                    UserAccounts.created_at
                ).join(
                    UserNames,
                    UserNames.user_id == UserAccounts.id
                )
    ...
    async def get_user_coordinates_by_id(
        self,
        id: int,
        session: AsyncSession
    ):
