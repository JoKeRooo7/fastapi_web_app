...
class UserRegistrationRepository:
    ...
    async def register_user(
        ...
        avatar_path: UploadFile,
        session: AsyncSession
    ):
        try:
            await self._check_existing_user(user.email, session)
            user_account, user_mail, user_name = await \
                self._create_user_records(user)
            await self._add_records_to_db(
                session,
                user_account,
                user_mail,
                user_name,
